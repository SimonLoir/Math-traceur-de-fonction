...
export default class parser {
    ...
    public getComputedValue(value: string) {
        ...
        if (value.indexOf("dérivée ") == 0) {
            value = this.stringify(
                math.derivate(this.exec(value.replace("dérivée ", "")))
            );
        } else if (value.indexOf("dérivée_seconde ") == 0) {
            value = this.stringify(
                math.derivate(
                    math.derivate(
                        this.exec(value.replace("dérivée_seconde ", ""))
                    )
                )
            );
        }
        ...
        for (var i = 0; i < exp.length; i++) {
            ...
            if (char == "(") {
                ...
                if (level == 0) {
                    ...
                    this.byIndexes2["$" + index] = this.stringify(
                        this.exec(inside)
                    );
    ...
    exec_and_sort(expression: string) {
        ...
        */
        ...
        if (sum_result["over"] != undefined) {
            if (
                Object.keys(sum_result["over"]).length == 1 &&
    ...
    public multiply(mult_result: any, div_result: any) {
        ...
        for (
            var index_mult_keys = 0;
            index_mult_keys < mult_keys.length;
            index_mult_keys++
        ) {
            ...
            if (e_mult != "over") {
                for (
                    var index_div_keys = 0;
                    index_div_keys < div_keys.length;
                    index_div_keys++
                ) {
        ...
        if (mult_result["over"] != undefined) {
            if (
                Object.keys(mult_result["over"]).length == 1 &&
